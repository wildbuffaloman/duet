...
function buildCard(canvasDir, filename, opts = {}) {
  ...
  // `real` is the card's canonical on-disk path: a symlinked card resolves to
  // the vault file it points at; a real card is itself. Clients copy this (FB-10).
  let real = full;
  ...
  const card = isImageFile(filename)
    ? buildImageCard(full, filename, stat)
    : buildHtmlCard(full, filename, stat);
  if (card) card.src = real; // never set on a blocked card (returned above)
  return card;
}
